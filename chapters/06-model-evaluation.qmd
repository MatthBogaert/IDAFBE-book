...
The previous two chapters turned a raw table into a clean, model-ready basetable. The
...
Get that wrong and every performance number you report afterwards is fictional and unreliable.
...
As soon as you compare several models or fine-tune hyperparameter
settings, a plain train/test split isn't
...
Cross-validation can play either role, or both at once, which gives three potential setups:
...
Hence, **fully nested cross-validation** removes the last dependence on a
...
- Decide on the time separation up front: know exactly which point in time separates "known" from
  "future" before touching the data and stick to this in your preprocessing and splitting.
...
where the total error is smallest. Increasing complexity beyond this (more variables, complex transformations, trees) makes things *worse*, not better [@james2013introduction].
...
To make this concrete, let's consider a synthetic time series example: `x` is a time index, `y` is
...
**Threshold-independent metrics**, ROC and AUC, precision-recall curves, and lift, look instead at
...
results. The false positive rate has a huge true-negative denominator, so it stays low even
...
The AUCs are close to each other and the ROC curves cross, exactly the ambiguous case the
...
mathematically special: @neslin2006defection show that a retention campaign's expected profit
